...
with torch.no_grad():
    for inputs, labels in test_loader:
        ...
        for idx in wrong_idx:
            misclassified_images.append(inputs[idx].cpu()) # 留在 CPU 用于画图
...
for i in range(8):
    ...
    ax.set_title(f"True: {true_label}\nPred: {pred_label}", color="red", fontsize=14, fontweight='bold')

plt.tight_layout()
plt.savefig('task6_misclassified.jpg')
plt.close()
print("已在当前目录保存 8 张错误分类样本图片: task6_misclassified.jpg")
...
plt.tight_layout()
plt.savefig('task7_confusion_matrix.jpg')
plt.close()
print("已在当前目录保存混淆矩阵图片: task7_confusion_matrix.jpg\n实验全部完成！")